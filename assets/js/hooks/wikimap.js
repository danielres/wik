...
const Wikimap = {
	...
	step() {
		// Single aggressive phase for faster settle
		const kRepel = 40000;
		const kSpring = 0.22;
		const damping = 0.86;
		const springLen = 90;
		...
		edges.forEach((e) => {
			...
			const force = kSpring * (dist - springLen);
		...
		if (this.energyBelowThresholdFor > 180 && this.rafId) {
			cancelAnimationFrame(this.rafId);
			this.rafId = null;
		}
	},
	...
	draw() {
		...
		// edges (lines)
		const edgeColor = "rgba(255,255,255,0.35)";
		...
		this.graph.nodes.forEach((n) => {
			const p = this.pos[n.id];
			ctx.beginPath();
			ctx.fillStyle = n.exists ? "#7dd3fc" : "#fca5a5";
			...
			ctx.fillText(n.label, (p.x + 10) * this.scale, p.y * this.scale);
			ctx.restore();
		});
		...
		this.graph.edges.forEach((e) => {
			...
			const tipX = b.x - ux * nodeR;
			const tipY = b.y - uy * nodeR;
			const baseX = tipX - ux * arrowSize;
			const baseY = tipY - uy * arrowSize;
			const leftX = baseX - uy * (arrowSize * 0.6);
			const leftY = baseY + ux * (arrowSize * 0.6);
			const rightX = baseX + uy * (arrowSize * 0.6);
			const rightY = baseY - ux * (arrowSize * 0.6);
